...
const EditUserModal: React.FC<EditUserModalProps> = ({
    ...
    const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (updateError) { // Limpiar error si el usuario empieza a escribir de nuevo
            clearUpdateError();
        }
    };
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-6 pb-3 border-b border-gray-200">
                    <h2 className="text-2xl font-semibold text-gray-800">Editar Nombre de Usuario</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="mb-4">
                    <p className="text-sm text-gray-600">
                        Editando a: <span className="font-semibold text-gray-800">{user.nombre} {user.apellido || ''}</span>
                    </p>
                    <p className="text-xs text-gray-500">
                        UsuarioID actual: <span className="font-mono bg-gray-100 px-1 rounded">{user.usuarioId}</span>
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    />
                    {updateError && <p className="text-sm text-red-600 bg-red-100 p-3 rounded-md border border-red-200">{updateError}</p>}
                    <div className="flex flex-col sm:flex-row-reverse gap-3 pt-3">
                        <Button
                            type="submit"
                            className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white"
                            isLoading={isUpdating}
                            disabled={isUpdating || !currentNombre.trim() || currentNombre.trim() === user.nombre}
                        >
                            Actualizar Nombre
                        </Button>
                        <Button
