...
const PlacedEmoji: React.FC<PlacedEmojiProps> = ({ position, quaternion, qrCodeData, category }) => {
    const router = useRouter();
    const [hovered, setHovered] = useState(false);
    const [color] = useState(Math.random() * 0xffffff); // Color aleatorio para diferenciar, mantenido con useState

    const handleSelect = () => {
        console.log(`Emoji seleccionado con qrCodeData: ${qrCodeData}, categoría: ${category}`);
    ...
    return (
        ...
        >
            ...
            >
                <meshStandardMaterial color={hovered ? 'hotpink' : new THREE.Color(color)} />
            </Box>
