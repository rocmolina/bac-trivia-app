...
} from "html5-qrcode"; // Eliminado Html5QrcodeResult ya que no se usa explícitamente
...
function PlayPageContent() {
  ...
  const [shouldRenderScannerContainer, setShouldRenderScannerContainer] =
    useState(false);
    ...
    if (html5QrCodeScannerRef.current) {
      try {
        html5QrCodeScannerRef.current
          .clear()
          .catch((err) =>
            console.warn("PlayPage: Advertencia al limpiar el escáner:", err),
          );
      } catch (e) {
        console.warn("PlayPage: Excepción al limpiar el escáner:", e);
      }
  ...
  useEffect(() => {
    if (shouldRenderScannerContainer && !html5QrCodeScannerRef.current) {
      if (!document.getElementById(qrcodeRegionId)) {
        console.error(
          "PlayPage: El div para el scanner QR no está montado todavía.",
        );
        setArError(
          "Error al preparar escáner. Haz clic de nuevo en 'Escanear QR'.",
        );
        setShouldRenderScannerContainer(false);
        return;
      }
      try {
        ...
        scannerInstance.render(
          ...
          (errorMessage) => {
            console.warn(
              "PlayPage: Error de scanner QR (puede ignorarse si es solo 'no QR found'):",
              errorMessage,
            );
          },
        );
        ...
        setArError(
          `Error del escáner QR: ${error.message || error.toString()}`,
        );
        setShouldRenderScannerContainer(false);
      }
    }
  ...
  useEffect(() => {
    ...
    return () => {
      stopScanner();
      if (arSession) {
        // && arSession.ended === false) {
        arSession
          .end()
          .catch((err) =>
            console.warn(
              "PlayPage: Error al terminar sesión AR en desmontaje:",
              err,
            ),
          );
      }
  ...
  const handleEnterAR = async () => {
    // Llamado desde el botón "Iniciar Experiencia AR"
    if (!scannedData || !isValidQr) {
      // Solo proceder si el QR es válido
    ...
    try {
      ...
      if (error.name === "NotSupportedError")
        message = "Este dispositivo o navegador no soporta AR.";
      else if (error.name === "SecurityError")
        message = "Permiso para AR denegado. Revisa HTTPS y permisos.";
  ...
  const TopRightButton = () => {
    if (arSession || shouldRenderScannerContainer || showQrResultScreen) {
      // Ocultar si está en cualquier estado post-inicial
      return null;
    }
    return (
      <Button
  ...
  return (
    ...
    >
      ...
      {arSession &&
        scannedData &&
        isValidQr === true && ( // Solo entrar a AR si el QR fue válido
          <ARCoreExperience
