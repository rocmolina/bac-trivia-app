...
function TriviaResultContent() {
    ...
    const categoryDetails = CATEGORIES.find(c => c.id.toLowerCase() === category.toLowerCase());
    const imageBaseUrl = categoryDetails?.svgUrl || '/icons/default.svg';
    const imageUrl = isSuccess ? imageBaseUrl : imageBaseUrl.replace('.svg', '_sad.svg');
    // Fallback si no existe la versión _sad, usa la normal
    const finalImageUrl = isSuccess ? imageBaseUrl : (
        (imageBaseUrl.includes('_sad.svg')) ? imageBaseUrl : imageBaseUrl.replace('.svg', '_sad.svg')
    );
    ...
    return (
        <div className={`flex flex-col items-center justify-center min-h-screen ${bgColor} p-6 sm:p-8 transition-colors duration-500`}>
            <div className={`w-full max-w-md text-center p-8 rounded-2xl shadow-2xl ${imageContainerBg}`}>
                <div className="mb-6 mx-auto flex items-center justify-center" style={{ width: imageSize, height: imageSize }}>
                    <Image
                        src={finalImageUrl}
                        ...
                        // Fallback si la imagen _sad no carga, se podría intentar la normal.
